var Snake = Snake || {};

Snake.gridTypes = { FLOOR: 0, WALL: 1, BODYX: 2, BODYY: 3, FOOD: 4, HEADRIGHT: 5, HEADLEFT: 6, HEADUP: 7, 
					HEADDOWN: 8, ESQUINASD: 9, ESQUINASI: 10, ESQUINAID: 11, ESQUINAII:12};
Snake.grid = [];
Snake.imageFiles = [];	
Snake.snake = [];
Snake.moveX = 0;
Snake.moveY = 0;
Snake.addTimer = 0;
Snake.gameOver = false;
Snake.score = 0;


Snake.init = function (canvas, level, loadingCompleteCallback) {
	// load canvas
	Snake.canvasElement = document.getElementById(canvas);
	Snake.context = this.canvasElement.getContext("2d");
	
	Snake.loadImages();
	Snake.loadGrid(level);
	Snake.createFood();
	Snake.registerEvents();	
	Snake.music = document.getElementById("musicaFondo");
	Snake.music.play();
		
	Snake.waitForImages(loadingCompleteCallback);
}

Snake.move = function() {
	var last = Snake.snake.length - 1;
	var lastPointX = Snake.snake[last].x;
	var lastPointY = Snake.snake[last].y
	
	// move snake's body
	var movX = Snake.moveX;
	var movY = Snake.moveY;
	for(var i = Snake.snake.length - 1; i > 0; i--) {
		if(i==1){
			if(Snake.moveX > 0 || Snake.moveX <0){
				Snake.snake[i].x = Snake.snake[i-1].x;
				Snake.snake[i].y = Snake.snake[i-1].y;
				Snake.snake[i].type = Snake.gridTypes.BODYX;
			}
			if(Snake.moveY > 0 || Snake.moveY <0){
				Snake.snake[i].x = Snake.snake[i-1].x;
				Snake.snake[i].y = Snake.snake[i-1].y;
				Snake.snake[i].type = Snake.gridTypes.BODYY;
			}
		}else{
				Snake.snake[i].x = Snake.snake[i-1].x;
				Snake.snake[i].y = Snake.snake[i-1].y;		
				Snake.snake[i].type = Snake.snake[i-1].type;
			}
		
				
		Snake.snake[i].redraw = true;
	}
	
	// move snake's head
	Snake.snake[0].x += Snake.moveX;
	Snake.snake[0].y += Snake.moveY;
	if(Snake.moveX > 0){
		Snake.snake[0].type = Snake.gridTypes.HEADRIGHT;	
	}else if(Snake.moveX < 0){
		Snake.snake[0].type = Snake.gridTypes.HEADLEFT;	
	}
	
	if(Snake.moveY > 0){
		Snake.snake[0].type = Snake.gridTypes.HEADDOWN;	
	}else if(Snake.moveY < 0){
		Snake.snake[0].type = Snake.gridTypes.HEADUP;	
	}
	Snake.snake[0].redraw = true;

	if(Snake.addTimer > 0) {
		Snake.snake.push(new Snake.gridItem(lastPointX, lastPointY, Snake.gridTypes.BODYX));	
		
		Snake.addTimer--;
	}
	
	Snake.checkCollisions();
}

Snake.checkCollisions = function() {
	// check for collision with food
	if(Snake.snake[0].x == Snake.food.x && Snake.snake[0].y == Snake.food.y) {
		Snake.addTimer = 2;
		if((Snake.level == Snake.levels.length-1) && Snake.score==4+Snake.level){
				Snake.drawEndGame();
		}
		Snake.createFood();
		Snake.score++;
		
		if(Snake.score == 5+Snake.level) {
			Snake.level++;			
			Snake.moveX = 0;
			Snake.moveY = 0;
			Snake.addTimer = 0;
			Snake.score = 0;
			Snake.loadGrid(Snake.level);		
			
		}
	}
	
	// check for collision with wall tile
	if(Snake.getPositionType(Snake.snake[0].x, Snake.snake[0].y) == Snake.gridTypes.WALL) {
		Snake.gameOver = true;
	}
	
	// check for collision with snake
	for(var i = 1; i < Snake.snake.length; i++) {
		if(Snake.snake[0].x == Snake.snake[i].x && Snake.snake[0].y == Snake.snake[i].y) {
			Snake.gameOver = true;
		}
	}
}